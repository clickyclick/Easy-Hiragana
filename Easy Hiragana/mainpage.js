...
if (currUrl.indexOf('#ja') > -1) { //checks if it is on japanese by the url 
    japanese = true;
} else {
    japanese = false;
}
...
$('#source').on('input', function () { //every type of input triggers box to be converted

    currUrl = window.location.href;
    if (currUrl.indexOf('#ja') > -1) { //checks if it is on japanese 
        japanese = true;
    } else {
        japanese = false;
    }

    if (this.value.length > 0 && japanese) {
